...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const populateOpt = populate || Step.associations
  ...
  add: async (values) => {
    ...
    return Step.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    ...
    const entry = await Step.updateOne(params, data, { multi: true });
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Step.attributes).reduce((acc, curr) => {
      switch (Step.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
